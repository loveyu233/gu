...
// Amount pg type: money
type Amount float64

func (c Amount) Value() (driver.Value, error) {
	amountStr := strconv.FormatFloat(float64(c), 'f', 2, 64)
	...
	for i, digit := range integerPart {
		...
		formattedIntegerPart += string(digit)
	}
	return "￥" + formattedIntegerPart + decimalPart, nil
}

func (c *Amount) Scan(value interface{}) error {
	q := value.(string)
	q = strings.Replace(q, "￥", "", -1)
	q = strings.Replace(q, ",", "", -1)
	float, err := strconv.ParseFloat(q, 32)
	if err != nil {
		return err
	}
	*c = Amount(float)
